...
# Versão: UV-v7.0 - Fix: usar uv pip install para wheel local
# ============================================================
...
echo "============================================================"
echo "[BOOT] SGLang Server - Ministral 3B (UV Edition v7)"
...
# --- ETAPA 2: AMBIENTE VIRTUAL COM UV (com seed para ter pip) ---
...
# --- ETAPA 6: SGL-KERNEL (usando uv pip install para arquivo local) ---
echo "[6/10] Instalando sgl-kernel (download direto)..."
WHEEL_NAME="sgl_kernel-0.3.11+cu124-cp310-abi3-manylinux2014_x86_64.whl"
curl -L -o "/tmp/$WHEEL_NAME" "https://github.com/sgl-project/whl/releases/download/v0.3.11/sgl_kernel-0.3.11%2Bcu124-cp310-abi3-manylinux2014_x86_64.whl"
uv pip install --no-cache --reinstall "/tmp/$WHEEL_NAME"
rm "/tmp/$WHEEL_NAME"

python3 -c "import sgl_kernel; print('✓ sgl_kernel instalado')"

# --- ETAPA 7: VLLM + SGLANG ---
echo "[7/10] Instalando vLLM e SGLang..."
uv pip install --no-cache "vllm==0.6.3.post1"

# Reinstalar PyTorch correto
...
import sys
print("\n" + "="*60)
print("VERIFICAÇÃO FINAL - UV Edition v7.0")
print("="*60)
