...
# Versão: UV-v11.0 - Desabilita FP8 + Remove sgl_kernel
# ============================================================
...
# DESABILITAR FP8 para evitar uso do sgl_kernel
export SGLANG_DISABLE_FP8=1

echo "============================================================"
echo "[BOOT] SGLang Server - Ministral 3B (UV Edition v11)"
echo "[INFO] PyTorch 2.4.1 | SGLang 0.4.3.post2 | CUDA 12.4"
echo "[INFO] FP8 DESABILITADO (modelo roda em FP16/BF16)"
echo "[INFO] A40 tem 48GB - Ministral 3B usa ~6GB em FP16"
echo "[DATA] $(date)"
echo "============================================================"
...
echo "[8/10] Removendo sgl_kernel..."
...
# Garantir numpy e PyTorch corretos
uv pip install --no-cache "numpy<2.0.0"
uv pip install --no-cache torch==2.4.1 --index-url https://download.pytorch.org/whl/cu124

# Remover sgl_kernel novamente
uv pip uninstall sgl-kernel sgl_kernel 2>/dev/null || true
rm -rf /workspace/venv/lib/python3.10/site-packages/sgl_kernel* 2>/dev/null || true
...
import sys
print("\n" + "="*60)
print("VERIFICAÇÃO FINAL - UV Edition v11.0")
print("="*60)
...
try:
    import sgl_kernel
    print(f"⚠ sgl_kernel: AINDA INSTALADO (pode causar erro)")
except ImportError:
    print(f"✓ sgl_kernel: REMOVIDO (correto para PyTorch 2.4.1)")
...
if errors:
    ...
    print("✓ FP8 desabilitado - modelo rodará em FP16/BF16")
print("="*60 + "\n")
VERIFY_EOF
...
echo "[INFO] Precisão: FP16/BF16 (FP8 desabilitado)"
echo "[INFO] Endpoint: http://${HOST}:${PORT}"
echo "============================================================"

# --disable-flashinfer-sampling evita uso de kernels que podem precisar de sgl_kernel
