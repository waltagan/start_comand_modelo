...
# Versão: UV-v8.0 - SEM sgl-kernel (incompatível com PyTorch 2.4)
# ============================================================
...
echo "============================================================"
echo "[BOOT] SGLang Server - Ministral 3B (UV Edition v8)"
echo "[INFO] Usando uv para instalações ultra-rápidas"
echo "[INFO] PyTorch 2.4.1 | SGLang 0.4.3.post2 | CUDA 12.4"
echo "[INFO] Nota: sgl-kernel OMITIDO (requer PyTorch 2.9+)"
echo "[DATA] $(date)"
echo "============================================================"

# --- ETAPA 1: DEPENDÊNCIAS DE SISTEMA + UV ---
echo "[1/9] Instalando dependências do sistema e uv..."
...
echo "[4/9] Instalando dependências base..."
uv pip install --no-cache \
    "numpy<2.0.0" \
    "transformers==4.48.3" \
...
# --- ETAPA 5: FLASHINFER ---
echo "[5/9] Instalando FlashInfer..."
uv pip install --no-cache flashinfer-python \
    --extra-index-url https://flashinfer.ai/whl/cu124/torch2.4

# --- ETAPA 6: VLLM + SGLANG ---
echo "[6/9] Instalando vLLM e SGLang..."
uv pip install --no-cache "vllm==0.6.3.post1"

# Reinstalar PyTorch correto (vLLM pode ter alterado)
uv pip install --no-cache --reinstall \
    torch==2.4.1 \
    --index-url https://download.pytorch.org/whl/cu124

# SGLang sem deps (vamos adicionar manualmente)
uv pip install --no-cache --no-deps "sglang[all]==0.4.3.post2"

# --- ETAPA 7: DEPENDÊNCIAS DO MODELO ---
echo "[7/9] Instalando suporte a modelos Mistral..."
uv pip install --no-cache \
...
# --- ETAPA 8: RUNTIME & UTILIDADES ---
echo "[8/9] Instalando dependências de runtime..."
uv pip install --no-cache \
    "pybase64>=1.4.0" \
    "orjson>=3.10.0" \
    "msgspec>=0.18.0" \
...
import sys
print("\n" + "="*60)
print("VERIFICAÇÃO FINAL - UV Edition v8.0")
print("="*60)
...
if "2.4.1" not in torch.__version__:
    errors.append(f"PyTorch deveria ser 2.4.1")
...
# sgl_kernel - OPCIONAL para SGLang 0.4.x
try:
    import sgl_kernel
    print(f"✓ sgl_kernel: {sgl_kernel.__version__}")
except ImportError:
    print(f"⚠ sgl_kernel: NÃO instalado (opcional para SGLang 0.4.x)")
